...
import '../../models/replay/replay_model.dart';
...
class FirebaseRemoteDataSourceImpl implements FirebaseRemoteDataSource {
  ...
  Stream<List<PostEntity>> readPosts(PostEntity post) {
    final postCollection = firebaseFirestore.collection(FirebaseConsts.posts).orderBy("createAt", descending: true);
  ...
  Future<void> createComment(CommentEntity comment) async {
    final commentCollection = firebaseFirestore.collection(FirebaseConsts.posts).doc(comment.postId).collection(FirebaseConsts.comment);
    ...
    try {
      ...
      if (!commentDocRef.exists) {
        commentCollection.doc(comment.commentId).set(newComment).then((value) {
          ...
          final postCollection = firebaseFirestore.collection(FirebaseConsts.posts).doc(comment.postId);
  ...
  Future<void> createReplay(ReplayEntity replay) async {
    final replayCollection = firebaseFirestore.collection(FirebaseConsts.posts).doc(replay.postId).collection(FirebaseConsts.comment).doc(replay.commentId).collection(FirebaseConsts.replay);
    ...
    try {
      ...
      if (!replayDocRef.exists) {
        replayCollection.doc(replay.replayId).set(newReplay).then((value) {
          final commentCollection = firebaseFirestore.collection(FirebaseConsts.posts).doc(replay.postId).collection(FirebaseConsts.comment).doc(replay.commentId);
  ...
  Stream<List<UserEntity>> getSingleOtherUser(String otherUid) {
    final userCollection = firebaseFirestore.collection(FirebaseConsts.users).where("uid", isEqualTo: otherUid).limit(1);
    return userCollection.snapshots().map((querySnapshot) => querySnapshot.docs.map((e) => UserModel.fromSnapshot(e)).toList());
  }
  ...
  Stream<List<CommentEntity>> readComments(String postId) {
    final commentCollection = firebaseFirestore.collection(FirebaseConsts.posts).doc(postId).collection(FirebaseConsts.comment).orderBy("createAt", descending: true);
  ...
  Stream<List<PostEntity>> readSinglePost(String postId) {
    final postCollection = firebaseFirestore.collection(FirebaseConsts.posts).orderBy("createAt", descending: true).where("postId", isEqualTo: postId);
    return postCollection.snapshots().map((querySnapshot) => querySnapshot.docs.map((e) => PostModel.fromSnapshot(e)).toList());
  }
